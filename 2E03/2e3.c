...
void WDT_ISR()
{
    ...
    while(i < 3)
    {
        if(ligne[i].size > 0)
        {
            TarefaAtual = retiraFila(&ligne[i]);
    ...
    /*if(ligne[0].size > 0) { //processar a fila de alta prioridade
    ...
    if(ligne[0].size == 0) //Se nao houver tarefas de alta prioridade, passa para a fila de baixa prioridade
    {
        ...
        if(ligne[1].size > 0) //Processar fila de baixa prioridade
        {
            TarefaAtual = retiraFila(&ligne[1]);
        }
    }
...
void startBRTOS()
{
    ...
    if(ligne[0].size > 0) { //pegar a pimeira tarefa para que TarefaAtual nao va para o endereco zero
        TarefaAtual = retiraFila(&ligne[0]);
    }
    else if(ligne[0].size == 0)
    {
        if(ligne[1].size > 0)
        {
            TarefaAtual = retiraFila(&ligne[1]);
